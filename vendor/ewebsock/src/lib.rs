...
pub struct Options {
    ...
    /// Transport settings for native targets.
    ///
    /// On web this field is not available.
    #[cfg(not(target_arch = "wasm32"))]
...
/// How a websocket should be transported on native platforms.
#[cfg(not(target_arch = "wasm32"))]
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum Transport {
    /// Direct TCP/TLS connections.
    Direct,
    /// Route connections through a SOCKS5 proxy.
    Socks(SocksOptions),
}

/// Configuration for SOCKS5 connectivity.
#[cfg(not(target_arch = "wasm32"))]
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SocksOptions {
    /// Where the SOCKS proxy is listening, e.g. `127.0.0.1:9150`.
    pub proxy_address: String,
    /// Optional username/password authentication.
    pub auth: Option<SocksAuth>,
}

/// Username/password credentials for SOCKS authentication.
#[cfg(not(target_arch = "wasm32"))]
#[derive(Clone, Debug, PartialEq, Eq)]
