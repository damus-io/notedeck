...
- **Preview system & theming** (`crates/notedeck_chrome/src/preview.rs`, `crates/notedeck/src/theme.rs`) centralize look-and-feel, font loading, and debug previews.
...
- **Responsive behavior**: Apps call `notedeck::ui::is_narrow` (for example, `crates/notedeck_columns/src/app.rs`) to adapt layouts for mobile widths; Chrome forwards the egui context so each app can decide how to adjust.
