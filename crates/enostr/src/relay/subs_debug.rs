...
use crate::{ClientMessage, Error, Note, RelayEvent, RelayMessage};
...
fn calculate_note_size(note: &Note) -> usize {
    let stack_size = mem::size_of_val(&note.id)
        + mem::size_of_val(&note.pubkey)
        + mem::size_of_val(&note.created_at)
        + mem::size_of_val(&note.kind)
        + mem::size_of_val(&note.tags)
        + mem::size_of_val(&note.content)
        + mem::size_of_val(&note.sig);

    let heap_size = note
        .tags
        .iter()
        .map(|tag| {
            tag.iter().map(|s| s.len()).sum::<usize>() + tag.len() * mem::size_of::<String>()
        })
        .sum::<usize>()
        + note.tags.len() * mem::size_of::<Vec<String>>()
        + note.content.len()
        + note.sig.len();
...
fn calculate_client_message_size(message: &ClientMessage) -> usize {
    match message {
        ClientMessage::Event { note } => mem::size_of_val(message) + calculate_note_size(note),
        ClientMessage::Req { sub_id, filters } => {
