    ...
    E: EventIngestCapability,
{
    ...
    pub async fn next(&mut self) -> OutboxServiceOutput {
        loop {
            ...
            tokio::select! {
                ...
                subnet = self.relay.network_watcher.next_down() => {
                    let output = self.apply_network_route_down(subnet);
                    if output.has_events() {
                        return output;
                    }
                    tokio::task::yield_now().await;
    ...
    /// Send the next due ping or disconnect the first leg whose last pong has
    /// exceeded the timeout. A timeout marks the relay disconnected and
    /// schedules its normal reconnect. One transition is applied per service
    /// turn.
    ...
    /// Disconnect connected websocket legs whose recorded local address belongs
    /// to `subnet`; legs routed elsewhere remain live. Route loss schedules the
    /// normal reconnect without recording a relay transport failure.
    fn apply_network_route_down(&mut self, subnet: IpNet) -> OutboxServiceOutput {
        let mut relays = self
            .relay
            .transport
            .websockets
            ...
            .filter_map(|(relay, leg)| {
                (leg.is_connected()
                    && leg
                        .local_addr
                        .is_some_and(|local| subnet.contains(&local.ip())))
                .then_some(relay.clone())
            })
            .collect::<Vec<_>>();
        relays.sort_unstable();
        ...
        for relay in relays {
            ...
            tracing::info!(
                relay = %relay,
                local = ?leg.local_addr,
                %subnet,
                "network route down, marking relay disconnected"
            );
            self.note_relay_route_lost(&relay, now);
            let status_output = self
                .relay
                .transport
                .set_status(relay.clone(), Some(RelayStatus::Disconnected));
            let close_output = self.apply_relay_transport_closed(&relay, leg.generation, now);
            output =
                merge_service_outputs(output, merge_service_outputs(status_output, close_output));
        }
        output
    }

    /// Record another open attempt in the relay's current reconnect streak.
    ...
    /// Finish the reconnect streak after a successful open and notify relay
    /// admission that the transport recovered.
    fn note_relay_transport_connected(&mut self, relay: &NormRelayUrl) {
        self.relay.transport.reconnects.remove(relay);
        self.relay.admission.note_transport_connected(relay);
    }

    /// Schedule reconnect after a transport closes. A failed opening attempt
    /// advances exponential backoff; an established leg that later disconnects
    /// restarts from the fixed reconnect delay and updates low-value admission.
    ...
    /// Schedule a normal reconnect after the local network route disappears.
    ///
    /// Route loss is not evidence that the relay failed, so it must not update
    /// relay-specific transport-failure health or low-value admission backoff.
    fn note_relay_route_lost(&mut self, relay: &NormRelayUrl, now: Instant) {
        self.relay.transport.reconnects.insert(
            relay.clone(),
            RelayReconnectState {
                attempt: 0,
                retry_at: now + self.relay.config.keepalive_reconnect_delay,
...
mod tests {
    ...
    #[tokio::test]
    async fn full_history_stays_unsettled_while_negentropy_session_is_open() {
        let (_relay_task, relay, captured, _notify) =
            crate::relay::test_utils::create_text_capture_relay().await;
        let mut service = OutboxService::with_capabilities(
            TestNip11Capability,
            EmptyFullHistoryCapability,
        ...
        loop {
            let neg_open_sent = captured
                .lock()
                .expect("lock captured text frames")
                .iter()
                .any(|text| text.starts_with("[\"NEG-OPEN\","));
            if neg_open_sent {
                break;
            }
            ...
            assert!(
                ...
                "timed out waiting for full-history NEG-OPEN"
            );
            let _ = tokio::time::timeout(Duration::from_millis(20), service.next()).await;
        }

        assert!(
            !service.full_history.full_history_catchup_complete(
                &service.pool,
                &service.negentropy,
                history_id,
            ),
            "full history settled while its relay negentropy session was still open"
        );
    }

    #[tokio::test]
    async fn full_history_declaration_does_not_emit_read_model_events() {
        let relay = relay("full-history-no-read-model-event");
        let mut service = service();
        let history_id = service.id_registry().next_full_history_id();
    ...
    #[tokio::test]
    async fn full_history_catchup_state_follows_service_lifecycle() {
        let (_relay_task, relay) = create_negentropy_eose_relay().await;
        let mut service = OutboxService::with_capabilities(
            TestNip11Capability,
            EmptyFullHistoryCapability,
        ...
        assert_eq!(service.end_effect_turn(), OutboxServiceOutput::NoEvents);
        assert!(!service.full_history.full_history_catchup_complete(
            &service.pool,
            &service.negentropy,
            history_id,
        ));
        ...
        let deadline = Instant::now() + Duration::from_secs(5);
        while !service.full_history.full_history_catchup_complete(
            &service.pool,
            &service.negentropy,
            history_id,
        ) {
            assert!(
                Instant::now() < deadline,
                "timed out waiting for full-history catchup"
            );
            let _ = tokio::time::timeout(Duration::from_millis(20), service.next()).await;
        }

        assert_eq!(
            service.clear_full_history(history_id),
            OutboxServiceOutput::NoEvents
        );
        assert!(service
            .full_history
            .full_history_snapshot(history_id)
            .is_none());
    }
    ...
    #[tokio::test]
    async fn route_down_disconnects_only_the_matching_connected_leg() {
        let matching = relay("route-down-matching");
        let unaffected = relay("route-down-unaffected");
        let mut service = service();
        install_connected_service_websocket(&mut service, matching.clone(), 17);
        install_connected_service_websocket(&mut service, unaffected.clone(), 18);
        service
            .relay
            .transport
            .websockets
            .get_mut(&matching)
            .expect("matching leg")
            .local_addr = Some("10.23.4.5:41000".parse().expect("socket address"));
        service
            .relay
            .transport
            .websockets
            .get_mut(&unaffected)
            .expect("unaffected leg")
            .local_addr = Some("192.168.8.9:42000".parse().expect("socket address"));

        let before_route_down = Instant::now();
        let output =
            service.apply_network_route_down("10.23.0.0/16".parse::<IpNet>().expect("subnet"));

        assert_eq!(
            output,
            OutboxServiceOutput::Events(vec![OutboxEvent::RelayStatusChanged {
                relay: matching.clone(),
                status: Some(RelayStatus::Disconnected),
            }])
        );
        assert!(!service.relay.transport.websockets.contains_key(&matching));
        assert!(service.relay.transport.websockets.contains_key(&unaffected));
        let reconnect = service
            .relay
            .transport
            .reconnects
            .get(&matching)
            .expect("matching relay reconnect state");
        assert_eq!(reconnect.attempt, 0);
        assert!(
            reconnect.retry_at
                >= before_route_down + service.relay.config.keepalive_reconnect_delay
        );
        assert!(!service.relay.transport.reconnects.contains_key(&unaffected));
    }
