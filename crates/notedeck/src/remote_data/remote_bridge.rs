...
impl BridgeOutboxSettlement {
    ...
    fn apply_scoped_outbox_ops(
        ...
        let outputs = self.outbox.apply_scoped_outbox_ops(scoped_outbox_ops);
        self.settle_outbox_outputs(outputs)
    }
        ...
        while let Some(action) = pending.pop_front() {
            let next_actions = match action {
                ...
                BridgeSettlementAction::ApplyScopedOutboxOps(scoped_outbox_ops) => {
                    self.settlement.apply_scoped_outbox_ops(scoped_outbox_ops)
                }
            };
