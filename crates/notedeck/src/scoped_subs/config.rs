...
/// Compare execution policy exactly while treating live and full-history
/// filter vectors as order-insensitive sets of canonical filter attributes.
...
pub enum ScopedSubReadiness {
    ...
    ///
    /// This occurs for account-scoped subs while switched away, or when an
    /// active config has no filters or resolves to no relay legs.
    Inactive,
