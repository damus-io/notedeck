...
impl AccountRelayData {
    ...
    pub fn query(&mut self, ndb: &Ndb, txn: &Transaction, keypair: &Keypair) {
        ...
        let private = self.query_private_relays(ndb, txn, keypair);
        if let Some(created_at) = private.created_at {
            self.private_updated_at = created_at;
        }
        self.private = private.relays;
    }
    ...
    /// projection changed. A kind-10002 delivery with the same relay set is a
    /// no-op, so callers do not retarget read-relay subscriptions unnecessarily.
    /// Idle frames do not clone relay state.
    #[profiling::function]
...
impl RelayAction {
    ...
    /// Whether this action targets the private kind-10013 list rather than the
    /// public kind-10002 list.
...
/// write relays and the private relays themselves, so the replacement is
/// recoverable from the relays it names. A read-only account cannot encrypt or
/// sign the replacement, so its in-memory edit remains local.
