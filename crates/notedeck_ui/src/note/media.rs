...
use egui::{
    pos2, Button, Color32, Context, CornerRadius, FontId, Image, Rect, Response, TextureHandle,
    Window,
};
use notedeck::{
    fonts::get_font_size, note::MediaAction, show_one_error_message, supported_mime_hosted_at_url,
    tr, GifStateMap, Images, JobPool, Localization, MediaCache, MediaCacheType, NotedeckTextStyle,
...
pub(crate) fn image_carousel(
    ...
    if show_popup {
        ...
        show_full_screen_media(ui, medias, img_cache);
    }
    action
}
...
fn show_full_screen_media(ui: &mut egui::Ui, medias: &[RenderableMedia], img_cache: &mut Images) {
    Window::new("image_popup")
        ...
        .show(ui.ctx(), |ui| {
            MediaViewer::new(medias).ui(img_cache, ui);
        });
}
...
struct MediaViewer<'a> {
    media: &'a [RenderableMedia<'a>],
}

impl<'a> MediaViewer<'a> {
    pub fn new(media: &'a [RenderableMedia]) -> Self {
        Self { media }
    }
    ...
    fn render_media(&self, images: &mut Images, ui: &mut egui::Ui) {
        for media in self.media {
            ...
            let texture = handle_repaint(
                ui,
                retrieve_latest_texture(image_url, cur_state.gifs, textured_image),
            );
