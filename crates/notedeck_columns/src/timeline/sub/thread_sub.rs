...
use crate::scoped_sub_owner_keys::{thread_owner_scope_key, ThreadOwnerId};
use crate::timeline::{
...
/// Thread subscription manager keyed by account and UI owner.
///
/// Each opened scope retains its local NostrDB subscription stack and one
/// scoped remote owner. Closing the top scope tears down its local state and
/// releases that remote owner. Scopes are removed from the top, so depth is
/// stable for the owner lifetime and forms part of its identity. Removing an
/// arbitrary lower scope would change every later depth and require extra
/// identity state.
#[derive(Default)]
...
impl ThreadSubs {
    ...
    pub(crate) fn dispose_route_for_account(
        ...
        let meta_id = ThreadOwnerId::Column(column_id);
        let Some(account_subs) = self.by_account.get_mut(&account_pk) else {
            return;
        };
        ...
        let owner_to_drop =
            thread_owner_scope_key(account_pk, meta_id, &scope.root_id, scope_depth as u64);
    ...
    pub(crate) fn get_local(
        ...
        self.get_local_for_owner(account_pk, ThreadOwnerId::Column(column_id))
    }

    fn get_local_for_owner(&self, account_pk: &Pubkey, meta_id: MetaId) -> Option<&Subscription> {
        self.by_account
            .get(account_pk)?
            .scopes
            .get(&meta_id)
    ...
    /// Return the selected account's local subscription for a thread owner.
    pub(crate) fn get_local_for_selected<'a>(
        &'a self,
        accounts: &Accounts,
        owner: impl Into<MetaId>,
...
mod tests {
    ...
    use crate::{
        deeplink::DeepLinkId,
        timeline::{thread::Threads, ThreadSelection},
    };
    ...
    fn thread_identity(
        ...
        ScopedSubIdentity::account(
            thread_owner_scope_key(account_pk, ThreadOwnerId::Column(col), root_id, scope_depth),
    ...
    #[test]
    fn thread_scope_owner_key_includes_owner_and_scope() {
        let account_pk = Pubkey::new([0x01; 32]);
        let root_id = NoteId::new([0x88; 32]);
        let column_a = ThreadOwnerId::Column(ColumnId::for_test(7));
        let column_b = ThreadOwnerId::Column(ColumnId::for_test(8));
        let deep_link = ThreadOwnerId::DeepLink(DeepLinkId::new(7));

        assert_ne!(
            thread_owner_scope_key(account_pk, column_a, &root_id, 0),
            thread_owner_scope_key(account_pk, column_a, &root_id, 1)
        );
        assert_ne!(
            thread_owner_scope_key(account_pk, column_a, &root_id, 0),
            thread_owner_scope_key(account_pk, column_b, &root_id, 0)
        );
        assert_ne!(
            thread_owner_scope_key(account_pk, column_a, &root_id, 0),
            thread_owner_scope_key(account_pk, deep_link, &root_id, 0)
        );
    }
