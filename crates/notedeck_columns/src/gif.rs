    ...
    match cached_image {
        ...
        TexturedImage::Animated(animation) => {
            ...
            let (texture, maybe_new_state, request_next_repaint) = match gifs.get(url) {
                Some(prev_state) => {
                    ...
                    if should_advance {
                        ...
                        match animation.get_frame(maybe_new_index) {
                            Some(frame) => {
                                // tracing::info!("animation {url} successfully increment");
                            ...
                            None => {
                                let (tex, state) =
                                    match animation.get_frame(prev_state.last_frame_index) {
                                        Some(frame) => {
                                            //     tracing::info!(
                                            //     "animation {url} new index {maybe_new_index} not ready, using old one"
                                            // );
                                            (&frame.texture, None)
                                        }
                                        None => {
                                            // tracing::info!("animation {url} new index nor old index ready. using first");
                                            (&animation.first_frame.texture, None)
                                        }
                                    };
                        ...
                        // tracing::info!("animation {url} no advance necessary");
                ...
                None => {
                    // tracing::info!("animation {url} gif state not found.");
                    (
