...
impl<'a> SettingsView<'a> {
    ...
    fn notifications_section(&mut self, ui: &mut egui::Ui) -> Option<SettingsAction> {
        ...
        settings_group(ui, title, |ui| {
            // Re-query OS permission state so the UI reflects changes
            // made in System Settings while the app is running
            notedeck::platform::refresh_notification_permission();
            ...
            // Permission status badge and request button (Android only)
            ...
            // On Android, gate on permission; on desktop, always enabled
            let enabled = if cfg!(target_os = "android") {
                permission_granted && !permission_pending
            } else {
                true
            };

            if cfg!(target_os = "android") {
                // Android: show all three delivery methods
                ui.label(richtext_small(tr!(
                    self.note_context.i18n,
                    "Delivery method:",
                ...
                // Desktop: simple Enabled/Disabled toggle
                // Native mode is used for desktop notifications

                if !permission_granted {
                    // Warning banner with deep link to macOS System Settings
                    Frame::new()
                        ...
                        .show(ui, |ui| {
                            ui.label(richtext_small(tr!(
                                self.note_context.i18n,
                                "macOS notification permission is required. Please enable notifications for Notedeck in System Settings.",
                                "macOS notification permission warning"
                            )));
                            if ui
                                .add(Button::new(richtext_small(tr!(
                                    self.note_context.i18n,
                                ...
                                .clicked()
                            {
                                if let Err(e) = robius_open::Uri::new(
                ...
                let mut selected: usize = if current_mode == NotificationMode::Disabled {
                    1
                } else {
                    0
                };
                ...
                if notification_radio_option(
                    ...
                    tr!(
                        self.note_context.i18n,
                        "Enabled",
                        "Desktop enabled notification option label"
                    ),
                    tr!(
                        self.note_context.i18n,
                        "Receive desktop notifications",
                        "Desktop enabled notification option description"
                    ),
                    enabled,
                ) {
                    action = Some(SettingsAction::SetNotificationMode(
            ...
            // Privacy info collapsible (Android only — compares FCM vs Direct Relay)
            if cfg!(target_os = "android") {
                ui.add_space(8.0);
                ui.collapsing(
                    richtext_small(tr!(
