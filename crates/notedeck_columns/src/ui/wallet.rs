use egui::Layout;
use notedeck::{jobs::Jobs, NoWallet, Wallet, WalletAction, WalletError, WalletState};
...
impl<'a> WalletView<'a> {
    pub fn new(state: &'a mut WalletState, jobs: &'a mut Jobs) -> Self {
        Self { state, jobs }
    }
    ...
    fn inner_ui(&mut self, ui: &mut egui::Ui) -> Option<WalletAction> {
        match &mut self.state {
            WalletState::Wallet(wallet) => show_with_wallet(ui, wallet, self.jobs),
...
fn show_with_wallet(
    ...
    ui.horizontal_wrapped(|ui| {
        let balance = wallet.get_balance(jobs);
