...
impl<'a> PublicationView<'a> {
    ...
    fn render_publication(
        ...
        ui.vertical(|ui| {
            ...
            let section_data: Option<(Vec<SectionData>, bool)> = self
                .publications
                .get(&self.selection.index_id)
                .map(|pub_state| {
    ...
    fn render_outline_view(
        ...
        for (child_idx, child_title, is_branch, is_resolved, note_key) in children {
            ...
            if let Some(a) = self.render_outline_child_card(
                ui,
                txn,
                state,
                child_idx,
    ...
    fn render_toc_tree(
        ...
        ui.horizontal(|ui| {
            ...
            let is_current = state.mode == ReaderMode::Paginated
                && leaf_index
                    .map(|i| i == state.current_leaf_index)
                    .unwrap_or(false);
